...
用法::
    ...
    python bayes_optlr_80100.py
    python bayes_optlr_80100.py --run-id test_80100
...
def main() -> int:
    ...
    parser.add_argument("--gun-config", type=Path, default=ROOT / "gun_modbus_config_bayes_80_100.json")
    ...
    try:
        for plan_index, (budget_name, round_index, max_iter, min_valid_evals) in enumerate(run_plan, 1):
            ...
            print("\n" + "#" * 72)
            print(
                f"[bayes-optlr-80100] 开始任务 {plan_index}/{len(run_plan)}："
                f"{budget_name} 第 {round_index}/{args.rounds_per_budget} 轮，"
                f"有效组至少 {min_valid_evals}，最多尝试 {max_iter} 组"
            )
            print(f"[bayes-optlr-80100] 本轮目录标识: {round_run_id}")
            print("#" * 72)
            ...
            if best is None:
                ...
                print(f"[bayes-optlr-80100] {budget_name} 第 {round_index} 轮结束，无成功结果")
                continue

            round_results.append((budget_name, round_index, round_run_id, best))
            print("\n" + "=" * 60)
            print(f"[bayes-optlr-80100] {budget_name} 第 {round_index}/{args.rounds_per_budget} 轮完成")
        ...
        print("\n" + "#" * 72)
        print(f"[bayes-optlr-80100] 全部任务结束，成功 {len(round_results)}/{len(run_plan)} 轮")

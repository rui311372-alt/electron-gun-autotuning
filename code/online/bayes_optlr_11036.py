...
用法::
    ...
    python bayes_optlr_11036.py --max-iter 80 --init-points 15
    python bayes_optlr_11036.py --max-iter 60 --plateau-patience 15
...
def _load_limits(gun_config_path: Path) -> dict[str, tuple[float, float] | list[float] | float]:
    ...
    return {
        ...
        "cathode_v": _get("cathode_v", (400.0, 750.0)),
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="每个 Ua/Ia 组合最多候选组数，范围 60--80（含舍弃组）",
    )
    ...
    parser.add_argument(
        "--gun-config",
        type=Path,
        default=ROOT / "gun_modbus_config_bayes_110_36.json",
        help="110/36 专用电子枪配置",
    )
    ...
    try:
        ...
        history_path = args.history or reports_dir / f"bayes_optlr_history_{opt.run_id}.json"
        best = opt.optimize(
            limits,

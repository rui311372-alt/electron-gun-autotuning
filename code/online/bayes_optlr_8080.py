...
用法::
    ...
    python bayes_optlr_8080.py
    python bayes_optlr_8080.py --run-id test_8080
...
_MIN_VALID_EVALUATIONS: int = 60
_MAX_EVALUATIONS: int = 60
_DEFAULT_ROUNDS: int = 2
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="每轮候选组数，8080 双轮程序固定为 60（含舍弃组）",
    )
    ...
    parser.add_argument(
        ...
        help="提前停止前至少需要的有效三拍组数，默认 60，确保每轮不会提前停止",
    )
    parser.add_argument(
    ...
    parser.add_argument("--gun-config", type=Path, default=ROOT / "gun_modbus_config_bayes_80_80.json")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="两轮运行的共同标识；实际目录自动添加 round_01/round_02",
    )
    parser.add_argument(
        "--rounds",
        type=int,
        default=_DEFAULT_ROUNDS,
        help="连续执行轮数（默认 2）；每轮使用完全独立的结果目录",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
    ...
    if args.max_iter != _MAX_EVALUATIONS:
        parser.error("8080 双轮程序的 max-iter 必须为 60")
    if args.min_valid_evals < _MIN_VALID_EVALUATIONS or args.min_valid_evals > args.max_iter:
        parser.error("8080 双轮程序的 min-valid-evals 必须为 60")
    if args.rounds <= 0:
    ...
    round_results: list[tuple[int, str, dict]] = []
    failed_rounds: list[int] = []

    try:
        for round_index in range(1, args.rounds + 1):
            round_run_id = f"{base_run_id}_round_{round_index:02d}"
            print("\n" + "#" * 72)
            print(
                f"[bayes-optlr-8080] 开始第 {round_index}/{args.rounds} 轮，"
                f"本轮尝试 {args.max_iter} 组"
            )
            print(f"[bayes-optlr-8080] 本轮目录标识: {round_run_id}")
            print("#" * 72)
            ...
            if args.history is None:
                ...
                history_path = args.history.with_name(
                    f"{args.history.stem}_round_{round_index:02d}{suffix}"
                )
            ...
            if best is None:
                ...
                print(f"[bayes-optlr-8080] 第 {round_index} 轮结束，无成功结果")
                continue

            round_results.append((round_index, round_run_id, best))
            print("\n" + "=" * 60)
            print(f"[bayes-optlr-8080] 第 {round_index}/{args.rounds} 轮完成")
        ...
        print("\n" + "#" * 72)
        print(f"[bayes-optlr-8080] 两轮任务结束，成功 {len(round_results)}/{args.rounds} 轮")
        for round_index, round_run_id, best in round_results:
            print(
                f"  第 {round_index} 轮: score={best['score']:.4f}, "
                f"目录={round_run_id}"
            )
        if failed_rounds:

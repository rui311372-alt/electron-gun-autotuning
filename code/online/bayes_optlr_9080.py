...
用法::
    ...
    python bayes_optlr_9080.py
    python bayes_optlr_9080.py --run-id test_9080
...
def main() -> int:
    ...
    parser.add_argument("--gun-config", type=Path, default=ROOT / "gun_modbus_config_bayes_90_80.json")
    ...
    try:
        for round_index in range(1, args.rounds + 1):
            ...
            print("\n" + "#" * 72)
            print(
                f"[bayes-optlr-9080] 开始第 {round_index}/{args.rounds} 轮，"
                f"本轮最多尝试 {args.max_iter} 组"
            )
            print(f"[bayes-optlr-9080] 本轮目录标识: {round_run_id}")
            print("#" * 72)
            ...
            if best is None:
                ...
                print(f"[bayes-optlr-9080] 第 {round_index} 轮结束，无成功结果")
                continue

            round_results.append((round_index, round_run_id, best))
            print("\n" + "=" * 60)
            print(f"[bayes-optlr-9080] 第 {round_index}/{args.rounds} 轮完成")
        ...
        print("\n" + "#" * 72)
        print(f"[bayes-optlr-9080] 两轮任务结束，成功 {len(round_results)}/{args.rounds} 轮")

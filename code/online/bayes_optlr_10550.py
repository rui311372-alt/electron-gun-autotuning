...
用法::
    ...
    python bayes_optlr_10550.py
    python bayes_optlr_10550.py --run-id test_10550
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="每轮最多候选组数，范围 60--80（默认 80，含舍弃组）",
    )
    ...
    parser.add_argument("--gun-config", type=Path, default=ROOT / "gun_modbus_config_bayes_105_50.json")
    ...
    if not _MIN_VALID_EVALUATIONS <= args.max_iter <= _MAX_EVALUATIONS:
        parser.error("max-iter 必须在 60 到 80 之间")
    if args.min_valid_evals < _MIN_VALID_EVALUATIONS or args.min_valid_evals > args.max_iter:
        parser.error("min-valid-evals 必须不小于 60，且不能大于 max-iter")
    if args.rounds <= 0:
    ...
    try:
        for round_index in range(1, args.rounds + 1):
            ...
            print("\n" + "#" * 72)
            print(
                f"[bayes-optlr-10550] 开始第 {round_index}/{args.rounds} 轮，"
                f"本轮最多尝试 {args.max_iter} 组"
            )
            print(f"[bayes-optlr-10550] 本轮目录标识: {round_run_id}")
            print("#" * 72)
            ...
            if best is None:
                ...
                print(f"[bayes-optlr-10550] 第 {round_index} 轮结束，无成功结果")
                continue

            round_results.append((round_index, round_run_id, best))
            print("\n" + "=" * 60)
            print(f"[bayes-optlr-10550] 第 {round_index}/{args.rounds} 轮完成")
        ...
        print("\n" + "#" * 72)
        print(f"[bayes-optlr-10550] 两轮任务结束，成功 {len(round_results)}/{args.rounds} 轮")
